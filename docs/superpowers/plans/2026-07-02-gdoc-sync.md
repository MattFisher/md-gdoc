...
Run: `pytest tests/test_binding.py -q`
Expected: FAIL / ERROR — `binding` has no attributes.
...
Run: `pytest tests/test_mdblocks.py -q`
...
Run: `pytest tests/test_mdblocks.py -q`
Expected: PASS. The list-nesting level arithmetic (`// 2`) and continuation handling are the likely iteration points — fix the implementation, not the tests.
...
- Produces:
  ...
  - Cell index formula, centralized in `_cell_index`:
    `table start = index + 1`; empty-cell paragraph of cell (r, c) in an R×C table starts at `index + 4 + r * (2 * C + 1) + 2 * c`.
    **The e2e round-trip test (Task 15) is the authority on this arithmetic** — if e2e shows an off-by-N, fix `_cell_index` and regenerate unit expectations; a comment in the code must say exactly this.
...
- Produces:
  ...
  - `cli.main(argv=None) -> int` — argparse subcommands:
    ...
    - `gdoc-sync pull <file>`
    - `gdoc-sync status <file>`
    Builds `GDocsApi(auth.get_credentials())` lazily (inside `main`, after arg parsing, so `--help` needs no credentials), prints human-readable outcomes, returns exit code 0 on success (SystemExit propagates for errors).
...
Run: `pytest -q`
Expected: all unit tests pass; e2e tests reported as skipped (`s` markers), zero network calls.

- [ ] **Step 4: Run e2e for real (requires OAuth client configured)**

Run: `RUN_GDOC_SYNC_E2E=1 python -m tests.e2e.test_end_to_end --generate` then `RUN_GDOC_SYNC_E2E=1 pytest tests/e2e -q`
Expected: golden file written; 4 e2e tests PASS.
