...
3. In the GitHub repo, create the environment: *Settings → Environments → New environment* → name it `pypi`. (Optionally add yourself as a required reviewer so releases need a manual approval click.)

The first tagged release creates the PyPI project and converts the pending publisher into a normal one.

## Each release

1. Update `__version__` in `src/md_gdoc/__init__.py` (single source of truth — `pyproject.toml` reads it).
